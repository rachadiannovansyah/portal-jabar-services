...
func NewUcase(cfg *config.Config, conn *utils.Conn, r *Repository, timeoutContext time.Duration) *Usecases {
	return &Usecases{
		...
		MasterDataServiceUsecase: _masterDataServiceUcase.NewMasterDataServiceUsecase(r.MasterDataServiceRepo, r.MainServiceRepo, r.ApplicationRepo, r.AdditionalInfRepo, cfg, timeoutContext),
	}
}
