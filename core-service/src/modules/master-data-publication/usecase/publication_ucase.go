...
func (n *masterDataPublicationUsecase) Store(ctx context.Context, body *domain.StoreMasterDataPublication) (err error) {
	...
	if body.DefaultInformation.Slug == "" {
		...
		body.DefaultInformation.Slug = helpers.SlugFormatter(body.DefaultInformation.Slug)

		slugExists := n.mdpRepo.SlugExists(ctx, body.DefaultInformation.Slug)
		if slugExists {
...
func (n *masterDataPublicationUsecase) Update(ctx context.Context, body *domain.StoreMasterDataPublication, pubID int64) (err error) {
	...
	// format slug request value
	body.DefaultInformation.Slug = helpers.SlugFormatter(body.DefaultInformation.Slug)
	if pub.DefaultInformation.Slug != body.DefaultInformation.Slug {
		slugExists := n.mdpRepo.SlugExists(ctx, body.DefaultInformation.Slug)
		if slugExists {
			return domain.ErrSlugAlreadyExist
		}
